...
class FNAgent():
    ...
    def save(self, model_path):
        self.model.save(model_path, overwrite=True, include_optimizer=True)
...
class Trainer():
    ...
    def train_loop(self, env, agent, episode=200, initial_count=-1,
        ...
        for i in range(episode):
            ...
            step_count = 0
            self.episode_begin(i, agent)  # self.loss = 0
            while not done:
                if render:
                    env.render()
                ...
                steering = agent.policy(s, env)
                a = [steering, throttle]

                n_state, reward, done, info = env.step(a)

                e = Experience(s, np.argmax(linear_bin(steering)), reward, n_state, done)
